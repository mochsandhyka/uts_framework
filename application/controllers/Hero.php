...
class Hero extends CI_Controller {
	...
	public function index($idJenis)
	{
		...
		$data["hero_list"] = $this->jenis_hero_model->getHeroByJenis($idJenis);
	...
	public function create($idJenis)
	{
		...
		if($this->form_validation->run()==FALSE){
            ...
            {
					$this->jenis_hero_model->insertHero($idJenis);
	...
	public function delete($idHero){
		...
		$this->load->library('form_validation');
		$this->load->model('jenis_hero_model');
		$this->jenis_hero_model->deleteHero($idHero);
		
		if($this->form_validation->run()==FALSE){
			redirect('hero/index/'.$this->uri->segment(3));
		}
	}
	public function update($idHero)
	{
		...
		$data['hero']=$this->jenis_hero_model->getHeroById($idHero);
		$data2=$this->jenis_hero_model->getHeroById($idHero);
		$nama = $data2[0]->foto;
